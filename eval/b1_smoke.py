"""
B1 smoke test — Track C errand 1 (see STATE.md).

Proves the three things the B1 model adapter (spec/20) will rely on:
...
2. Store the key in the OS credential store (spec/50 rule 10) — never setx, never a
   repo file. Same command on Windows (Credential Manager) and macOS (Keychain):
     pip install keyring
...
DONE-WHEN (record in STATE.md Track C):
  [ ] exits 0 on the Windows PC (all three PASS)
  [ ] time-to-first-token noted (expect roughly 300–900 ms; it feeds the spec/40 budget)
...
# in shared/schemas/tools.json (`parameters`, plus `tier`). This comment used to claim they were the
...
def get_key() -> str:
    """Credential store first (spec/50 rule 10); session env var as fallback."""
    try:
        import keyring
...
def main() -> None:
    ...
    print("\nAll green — B1 is ready. Record the first-token number in STATE.md "
          "Track C and mark errand 1 done.")
